...
export const OPR_RULES = {
  ...
  // ═══════════════════════════════════════════════════════════════════════════
  // BLESSED SISTERS RULES — added from Blessed Sisters v3.5.2
  // ═══════════════════════════════════════════════════════════════════════════

  // ── Army-Wide: Devout ─────────────────────────────────────────────────────
  // Same mechanic as Surge weapon rule, but applied as a unit special rule
  // (applies to all weapons). Engine checks unit.hasRule('Devout') when
  // resolving attack hits.
  ...
  Devout: {
    ...
    hooks: {
      [HOOKS.AFTER_HIT_ROLLS]: ({ hitRolls, unit, specialRulesApplied }) => {
        ...
        // Extra hits are plain hits — NOT counted as 6s for further special rules.
  ...
  'Devout Boost': {
    ...
    hooks: {},
    // Engine: Devout hook reads unit.hasRule('Devout Boost') to set threshold = 5.
  },
  ...
  // ── Guarded ───────────────────────────────────────────────────────────────
  // Same mechanic as Sturdy (Dwarf Guilds) but uses 9" distance threshold.

  Guarded: {
    description: 'When shot or charged from over 9" away, get +1 to defense rolls.',
    hooks: {
      [HOOKS.BEFORE_SAVE_DEFENSE]: ({ attackDistance, isMelee, defense, specialRulesApplied }) => {
        // For melee (charge), also check distance if charger started 9"+ away.
  ...
  'Guarded Buff': {
    ...
    hooks: {},
    // AI sets target.guarded_buff_active = true.
    // BEFORE_SAVE_DEFENSE checks target.guarded_buff_active and applies +1 defense vs distant attacks.
  },
  ...
  'Piercing Assault': {
    ...
    // Engine: pass isCharging flag in melee context (attacker charged this activation).
  },
  ...
  // ── Point-Blank Surge ─────────────────────────────────────────────────────
  // Same as Devout but only triggers when shooting within 12".
  ...
  'Casting Debuff': {
    ...
    hooks: {},
    // AI sets target.casting_debuff_penalty += 1 (or sets a flag).
    // Spell casting resolution checks target.casting_debuff_penalty and subtracts from cast roll.
    // Engine: pass casting_debuff_penalty context into spell resolution.
  },
  ...
  'Courage Buff': {
    ...
    hooks: {},
    // AI sets target.courage_buff_active = true.
    // ON_MORALE_TEST checks courage_buff_active and returns +1 roll bonus (single use).
  },
  ...
  'Precision Shooter Buff': {
    ...
    hooks: {
      [HOOKS.BEFORE_HIT_QUALITY]: ({ unit, quality, isMelee, specialRulesApplied }) => {
        ...
        unit.precision_shooter_buff_active = false; // consume
        specialRulesApplied.push({ rule: 'Precision Shooter Buff', value: 1, effect: '+1 to hit (shooting, buffed)' });
        return { quality: Math.max(2, quality - 1) };
      },
    },
    // AI sets target.precision_shooter_buff_active = true at activation.
  },
  ...
  'Precision Fighter Buff': {
    ...
    hooks: {
      [HOOKS.BEFORE_HIT_QUALITY]: ({ unit, quality, isMelee, specialRulesApplied }) => {
        ...
        unit.precision_fighter_buff_active = false; // consume
        specialRulesApplied.push({ rule: 'Precision Fighter Buff', value: 1, effect: '+1 to hit (melee, buffed)' });
        return { quality: Math.max(2, quality - 1) };
      },
    },
    // AI sets target.precision_fighter_buff_active = true at activation.
  },

  // ── Precision Target(X) ───────────────────────────────────────────────────
  // Similar to Piercing Tag / Piercing Spotter but gives +X to HIT ROLLS, not AP.
  ...
  'Precision Target': {
    ...
    hooks: {
      [HOOKS.BEFORE_HIT_QUALITY]: ({ target, quality, specialRulesApplied }) => {
        ...
        if (markers <= 0) return {};
        target.precision_target_markers = 0; // consume all markers on first attack
        specialRulesApplied.push({ rule: 'Precision Target', value: markers, effect: `+${markers} to hit (precision target markers)` });
        return { quality: Math.max(2, quality - markers) };
      },
    },
    // AI/Engine: once per game activation, set target.precision_target_markers = _ruleParamValue.
    // unit._precisionTargetUsed flag.
  },

  // ── Purge ─────────────────────────────────────────────────────────────────
  // Ignores Regen (like Rupture/Quake) + conditional AP+1 vs medium-armor targets.
  ...
  Purge: {
    ...
    hooks: {
      ...
      [HOOKS.BEFORE_SAVE_DEFENSE]: ({ targetDefense, ap, specialRulesApplied }) => {
        // targetDefense is the numeric save (2, 3, 4 = 2+, 3+, 4+ respectively).
        // Defense 2+ to 4+ means numeric value 2, 3, or 4.
    ...
    // Engine: pass targetDefense (numeric save value) in context.
    // Also needs targetDefense = majority defense of target unit.
  },
  ...
  // ── Fortified / Aura ──────────────────────────────────────────────────────
  // Fortified appeared in Dwarf Guilds context — re-declare here for completeness.
  // Incoming hits count as AP(-1), minimum AP(0).
