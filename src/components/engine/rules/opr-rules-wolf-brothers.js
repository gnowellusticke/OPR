/**
 * rules/opr-rules-wolf-brothers.js
 * Wolf Brothers faction rules from v3.5.2 (page 3)
...
export const WOLF_BROTHERS_RULES = {
  ...
  // -------------------------------------------------------------------------
  Wolfborn: {
    description: 'When this unit is activated, you may place all models with this rule in it anywhere fully within 3" of their position.',
    hooks: {
      [HOOKS.ON_ACTIVATION_START]: ({ unit, specialRulesApplied }) => {
        if (unit._wolfbornUsed) return {};
        unit._wolfbornUsed = true;
        specialRulesApplied.push({ rule: 'Wolfborn', effect: 'may reposition up to 3"' });
        return { boundingMove: { distance: 3 } };
      },
      [HOOKS.ON_ACTIVATION_END]: ({ unit }) => {
        delete unit._wolfbornUsed;
      },
    },
  },
  ...
  'Versatile Attack': {
    ...
    hooks: {
      [HOOKS.ON_ACTIVATION_START]: ({ unit, specialRulesApplied }) => {
        const mode = 'hit'; // default; player would choose
  ...
  'Courage Aura': {
    hooks: { [HOOKS.ON_GET_RULES]: () => ({ additionalRules: ['Courage'] }) },
  },
  Courage: {
    hooks: {
  ...
  'Wolf Sight': {
    ...
    hooks: {
      [HOOKS.ON_SPELL_CAST]: ({ caster, target, specialRulesApplied }) => {
        if (!target) return {};
        target._unstoppableMarked = true;
        specialRulesApplied.push({ rule: 'Wolf Sight', effect: `marked ${target.name}` });
        return {};
      },
    },
  },
  ...
  'Wolf Trauma': {
    ...
    hooks: {
      [HOOKS.ON_SPELL_CAST]: ({ caster, target, specialRulesApplied }) => {
        if (!target) return {};
        specialRulesApplied.push({ rule: 'Wolf Trauma', effect: `1 hit (Smash) on ${target.name}` });
        return {
  ...
  'Godly Thunder': {
    ...
    hooks: {
      [HOOKS.ON_SPELL_CAST]: ({ caster, target, specialRulesApplied }) => {
        if (!target) return {};
        specialRulesApplied.push({ rule: 'Godly Thunder', effect: `2 hits (AP4) on ${target.name}` });
        return {
          extraHits: [{ target, count: 2, ap: 4 }]
        };
      },
    },
  },
  ...
  'Storm of Power': {
    description: 'Pick one enemy unit within 12", which takes 6 hits with AP(1) and Surge.',
    hooks: {
      [HOOKS.ON_SPELL_CAST]: ({ caster, target, specialRulesApplied }) => {
        if (!target) return {};
        specialRulesApplied.push({ rule: 'Storm of Power', effect: `6 hits (AP1, Surge) on ${target.name}` });
        return {
          extraHits: [{ target, count: 6, ap: 1, surge: true }]
        };
      },
    },
  },
