/**
 * rules/opr-rules-battle-brothers.js
 * Battle Brothers faction rules from v3.5.2
 */
import { HOOKS } from '../RuleRegistry.js';

export const BATTLE_BROTHERS_RULES = {
  // Army-wide
  Battleborn: {
    ...
    hooks: {
      [HOOKS.ON_ROUND_START]: ({ unit, dice, specialRulesApplied }) => {
        ...
        if (roll >= 4) {
          specialRulesApplied.push({ rule: 'Battleborn', effect: `recovered (rolled ${roll})` });
          return { clearShaken: true };
        } else {
          specialRulesApplied.push({ rule: 'Battleborn', effect: `failed recovery (rolled ${roll})` });
          return {};
        }
      },
    },
  ...
  'Melee Shrouding': {
    ...
    hooks: {
      ...
      [HOOKS.MODIFY_SPEED]: ({ unit, action, speedDelta, specialRulesApplied }) => {
        ...
        // Check if the target (the unit being charged) has this rule.
        // But we don't have target in MODIFY_SPEED. So we need a different approach.
        // We'll use a separate hook ON_CHARGE_DISTANCE, but we don't have that.
        // For now, we'll assume the engine checks this rule in charge movement.
  ...
  'Re-Deployment': {
    ...
    hooks: {
      [HOOKS.AFTER_DEPLOYMENT]: ({ gameState, specialRulesApplied }) => {
        // This rule would need to allow the player to select units to redeploy.
        // We'll return a list of units that can be redeployed.
        // The engine should handle the actual redeployment.
    ...
    hooks: {
      ...
      [HOOKS.BEFORE_SAVE_DEFENSE]: ({ target, ap, specialRulesApplied }) => {
        if (target?.unstoppable_marked) {
          ...
          // Unstoppable means negative AP modifiers don't apply. So we set ap to at least 0.
  ...
  'Advanced Sight': {
    ...
    hooks: {
      [HOOKS.ON_SPELL_CAST]: ({ caster, target, specialRulesApplied }) => {
        if (target) {
          target.unstoppable_marked = true;
          specialRulesApplied.push({ rule: 'Advanced Sight', effect: `marked ${target.name}` });
        }
      },
    },
  },
  'Cerebral Trauma': {
    ...
    hooks: {
      [HOOKS.ON_SPELL_CAST]: ({ caster, target, specialRulesApplied }) => {
        if (target) {
          specialRulesApplied.push({ rule: 'Cerebral Trauma', effect: `1 hit with Smash on ${target.name}` });
  ...
  'Protective Dome': {
    ...
    hooks: {
      [HOOKS.ON_SPELL_CAST]: ({ caster, gameState, specialRulesApplied }) => {
        ...
        specialRulesApplied.push({ rule: 'Protective Dome', effect: `gave Evasive to ${friendlies.length} units` });
      },
    },
  },
  'Psychic Terror': {
